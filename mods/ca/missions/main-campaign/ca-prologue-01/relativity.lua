...
WorldLoaded = function()
	...
	Trigger.OnKilled(SubPen, function(self, killer)
		if ObjectiveDestroySubPen == nil then
			ObjectiveDestroySubPen = Greece.AddObjective("摧毁苏联潜艇船坞。")
		end
	...
	Trigger.AfterDelay(DateTime.Seconds(30), function()
		Tip("选中任意单个单位或建筑时，屏幕右下角会显示其强弱属性（需在设置中启用「选中单位提示」）。")
	end)
...
SendCruisers = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(4), function()
		Media.DisplayMessage("遭遇苏联海军！我们正遭受猛烈炮火！", "Cruiser Captain", HSLColor.FromHex("99ACF2"))
		MediaCA.PlaySound(MissionDir .. "/encountering.aud", 2)
		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(4)), function()
			Media.DisplayMessage("这不可能！这片水域已被清理干净！", "Cruiser Captain", HSLColor.FromHex("99ACF2"))
