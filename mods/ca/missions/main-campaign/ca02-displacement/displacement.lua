...
WorldLoaded = function()
	...
	ObjectiveClearPath = Greece.AddObjective("为即将抵达的运输车队清出一条路。")

	if IsHardOrBelow() then
		ObjectiveProtectConvoys = Greece.AddObjective("损失不得超过" .. MaxLosses[Difficulty] .. " convoy trucks.")
	else
		ObjectiveProtectConvoys = Greece.AddObjective("不得损失任何运输卡车。")
	end
	...
	if TimerTicks == 0 then
		if MaxLosses[Difficulty] == 0 then
			...
			if TrucksLost == MaxLosses[Difficulty] then
				UserInterface.SetMissionText("保护车队。不能再损失任何卡车。", HSLColor.Yellow)
			else
				UserInterface.SetMissionText("保护车队。可接受损失：" .. TrucksLost .. " / " ..  MaxLosses[Difficulty] , HSLColor.Yellow)
			end
		end
	else
		UserInterface.SetMissionText("下一批车队抵达倒计时：" .. UtilsCA.FormatTimeForGameSpeed(TimerTicks), HSLColor.Yellow)
	end
end
