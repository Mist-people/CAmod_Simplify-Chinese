...
WorldLoaded = function()
	...
	ObjectiveDestroyMothership = GDI.AddObjective("摧毁思金母舰。")

	Trigger.OnAllKilledOrCaptured(NWReactors, function()
		ScrinDefenseBuff1.Destroy()
		Notification("西北方反应堆已被摧毁。思金防御已被削弱。")
	...
	Trigger.OnAllKilledOrCaptured(NEReactors, function()
		...
		Notification("东北方反应堆已被摧毁。思金防御已被削弱。")
	...
	Trigger.AfterDelay(DateTime.Seconds(5), function()
		Media.DisplayMessage("开始攻击波次。让我们看看面对的是什么。完毕。", "GDI飞行员", HSLColor.FromHex("F2CF74"))
	...
	Trigger.AfterDelay(DateTime.Seconds(10), function()
		...
		Trigger.AfterDelay(DateTime.Seconds(15), function()
			Media.DisplayMessage("我们几乎没造成任何伤害！你需要先把那些护盾打下来。完毕。", "GDI飞行员", HSLColor.FromHex("F2CF74"))
	...
	Trigger.OnKilled(SignalTransmitter, function(self, killer)
		...
		if ObjectiveHackSignalTransmitter ~= nil and not GDI.IsObjectiveCompleted(ObjectiveHackSignalTransmitter) then
			...
			Trigger.AfterDelay(DateTime.Seconds(2), function()
				Media.DisplayMessage("信号发射器已被摧毁！你现在唯一的选择是用蛮力打掉那些护盾。我只希望你能及时做到。", "Nod指挥官", HSLColor.FromHex("FF0000"))
			end)
		end
	end)
	...
	Utils.Do(cyborgs, function(c)
		...
		Trigger.OnDamaged(c, function(self, attacker, damage)
			if not SleepingCyborgsMessageShown and not Mothership.IsDead and not self.IsDead and self.Health < self.MaxHealth * 0.8 then
				...
				Notification("Nod半机械人似乎处于休眠状态。浓缩泰伯利亚提供了强大的再生能力。建议不要接敌。")
			...
			Trigger.AfterDelay(DateTime.Seconds(10), function()
				...
				Trigger.AfterDelay(DateTime.Seconds(15), function()
					if not Mothership.IsDead then
						Notification("攻击波次成功。母舰船体遭受重创。下一波攻击预计2分钟后。")
						...
						Trigger.AfterDelay(DateTime.Minutes(2), function()
							...
							Trigger.AfterDelay(DateTime.Seconds(15), function()
								if not Mothership.IsDead then
									Notification("预计还需一次攻击即可摧毁母舰，预计2分钟后。")
...
DropHackers = function()
	...
	if not FirstHackersArrived then
		...
		Media.DisplayMessage("注意GDI指挥官。我们正在派遣一些黑客给你。用他们骇入思金信号发射器。他们将能为你降下母舰的护盾。", "Nod指挥官", HSLColor.FromHex("FF0000"))
	else
		MediaCA.PlaySound(MissionDir .. "/seth_morehackers.aud", 2)
		Media.DisplayMessage("我们正在派遣另一队黑客给你。或许这次你会更小心。", "Nod指挥官", HSLColor.FromHex("FF0000"))
	end
		...
		Trigger.AfterDelay(DateTime.Seconds(5), function()
			...
			if not FirstHackersArrived then
				if not SignalTransmitter.IsDead then
					ObjectiveHackSignalTransmitter = GDI.AddSecondaryObjective("骇入信号发射器以降下护盾。")
				end
...
InitChronoTanks = function()
	Trigger.AfterDelay(ChronoTanksDelay[Difficulty], function()
		...
		Notification("盟军提供了一支时空坦克中队。用它们摧毁东北方的思金反应堆。")
...
InitMADTankAttack = function()
	Trigger.AfterDelay(MADTankAttackDelay - DateTime.Seconds(20), function()
		...
		Notification("侦测到信号弹。苏联正派遣一辆MAD坦克摧毁西北方的思金反应堆。他们请求汇合以提供护航。")
...
DoFinale = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(6), function()
		...
		Media.DisplayMessage("指挥官，我们终于见面了！你的贡献极其宝贵，尽管你可能并非有意。", "凯恩", HSLColor.FromHex("FF0000"))
		...
		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(6)), function()
			Media.DisplayMessage("很讽刺不是吗？GDI竟然为兄弟会的最终胜利铺平了道路。", "凯恩", HSLColor.FromHex("FF0000"))
		end)

		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(12)), function()
			Media.DisplayMessage("当然盟军和苏联也各尽其职。", "凯恩", HSLColor.FromHex("FF0000"))
		end)

		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(16)), function()
			Media.DisplayMessage("我苦心经营的时空操纵终于开花结果，现在我们站在了门槛之上。", "凯恩", HSLColor.FromHex("FF0000"))
		end)

		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(21)), function()
			Media.DisplayMessage("还有很多事要做。我毫不怀疑我们会再次相遇。", "凯恩", HSLColor.FromHex("FF0000"))
		end)
